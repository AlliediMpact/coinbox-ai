...
export async function validatePaymentAttempt(userId: string): Promise<boolean> {
    ...
    try {
        return await adminDb.runTransaction(async (transaction: any) => {
            ...
            const firstAttemptTime = data.firstAttempt instanceof Date 
                ? data.firstAttempt.getTime() 
                : (data.firstAttempt as any).toDate().getTime();
                
            const timeDiff = now.getTime() - firstAttemptTime;
            ...
            if (data.count >= MAX_ATTEMPTS) {
                // Log suspicious activity (fire and forget, outside transaction or separate)
                // We can't easily do fire-and-forget inside a transaction for a different doc 
                // without including it in the transaction, which might lock it.
