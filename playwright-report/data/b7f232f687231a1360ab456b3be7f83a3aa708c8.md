...
- Name: User Onboarding Journey >> Verified user should see the onboarding walkthrough
- Location: /workspaces/coinbox-ai/src/e2e-tests/onboarding.e2e.spec.ts:51:7

# Error details

```
Error: browserType.launch: Executable doesn't exist at /home/codespace/.cache/ms-playwright/chromium_headless_shell-1169/chrome-linux/headless_shell
╔═════════════════════════════════════════════════════════════════════════╗
...
>  51 |   test('Verified user should see the onboarding walkthrough', async ({ page }) => {
  ...
  123 |     
  124 |     // The onboarding dialog should be visible again
  125 |     await expect(page.getByText(/get started with coinbox/i)).toBeVisible();
  126 |     
  127 |     // Check that the progress indicator was reset
  128 |     await expect(page.getByText(/0% complete/i)).toBeVisible();
  129 |   });
  130 | });
  131 |
```